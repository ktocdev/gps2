...
- Activity Log FAB with Draggable Panel DONE
...
| Icon | Action | Color | Functionality |
|------|--------|-------|---------------|
| 📜 | Activity Log | Yellow | Opens draggable Activity Feed panel |
...
- [X] Add 📜 Activity Log button on left side
- [X] Style as circular buttons with hover/active states
- [X] Position fixed to canvas edges (right for actions, left for log)
...
- `.game-fab-container` - Right FAB container
- `.game-fab-container--left` - Left FAB container (Activity Log)
...
### Activity Log FAB & Panel
**Priority:** MEDIUM (P2)
**Status:** ✅ COMPLETE
**Category:** UI/UX

**Goal:** Provide access to activity messages via a draggable, resizable panel

**Implementation:**
1. [X] Added 📜 FAB button on left side of canvas (yellow color)
2. [X] Created Activity Feed panel component
3. [X] Panel is draggable (click and drag header)
4. [X] Panel is resizable (drag corner handle)
5. [X] Connected to `loggingStore.activityMessages`
6. [X] Category-based color coding for messages
7. [X] Time formatting for timestamps

**Panel Features:**
| Feature | Implementation |
|---------|----------------|
| Draggable | Mouse events on header |
| Resizable | Corner resize handle |
| Default Position | (70px, 50px) |
| Default Size | 320x400px |
| Min Size | 200x150px |
...
**CSS Classes:**
- `.activity-feed-panel` - Main panel container
- `.activity-feed-panel__header` - Draggable header (yellow background)
...
- `.activity-feed-panel__resize` - Corner resize handle

**Files Modified:**
- [Habitat3DDebug.vue](../../src/components/debug/environment/Habitat3DDebug.vue) - Added Activity Feed panel, state, handlers, CSS
