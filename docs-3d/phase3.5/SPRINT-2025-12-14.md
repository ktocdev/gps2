...
- 3D Inventory Item Placement (Click-to-Place) PLANNED
...
**Implementation Checklist:**
- [ ] Add `placementMode` state to Habitat3DDebug.vue
- [ ] Add `worldToGrid()` function to shared/utils.ts
- [ ] Create `enterPlacementMode(itemId)` function
- [ ] Create ghost preview mesh with transparency
- [ ] Add `@mousemove` handler to canvas for preview positioning
- [ ] Add `isValidPlacement()` validation function
- [ ] Update preview color (green=valid, red=invalid) via emissive
- [ ] Modify `handleCanvasClick()` to handle placement mode
- [ ] Add `exitPlacementMode()` function
- [ ] Update ESC key handler (close inventory first, then cancel placement)
