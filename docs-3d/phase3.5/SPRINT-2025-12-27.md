...
- [x] **Sidebar button icon** - Changed from 📦 to ☰, title to "Control Panel"
...
- [x] **Interact FAB & Subnav Menu** - Renamed Guinea Pigs FAB to Interact (💛), created FabSubnavMenu popover with 7 actions (Pet, Hold, Hand Feed, Gentle Wipe, Show Toy, Peek-a-Boo, Talk To)
...
- [x] **Pet Interaction Need Fulfillment** - Petting now satisfies social need (+25) and increases friendship (+2), logs activity with 🫳 emoji
...
**Fixes Applied:**
- [x] `stats.css` - Fixed `border-bottom` → `border-block-end`
...
### Enhanced Habitat Physics (Prerequisite for Play/Chew) ✅
**Priority:** HIGH (P1)
**Category:** Feature
**Status:** COMPLETE
...
- [x] **Playing** - Find toys, play animation, satisfy play need ✅
...
- [x] **Chewing** - Gnaw on sticks/chews, item pinned to mouth ✅
...
- `finishDrinking()` - logs drinking water 💧
- `finishGrooming()` - logs grooming ✨
- `finishSheltering()` - logs leaving shelter 🏠
- `finishSleeping()` - logs waking up 😴
- `checkAutonomousPooping()` - logs pooping 💩 (as environmental event)

**2. Player Action Logging** (`Habitat3DDebug.vue`):
- `fabQuickClean()` - logs quick clean result 🧽
- `fabRefillWater()` - logs water refill with amount 💧
- `handleRefillWater()` - logs water refill from bottle menu 💧
- `handleContainerFill()` - logs filling hay rack 🌾
- `handleContainerClear()` - logs clearing hay rack 🌾 or food bowl 🥗
- `handleAddItemToContainer()` - logs adding food to bowl (with food name/emoji) or hay to rack 🌾

**3. Hay Management Dialog Logging** (`HayManagementDialog.vue`):
- `handleAddServing()` - logs adding hay to rack 🌾
- `handleClearRack()` - logs clearing hay rack 🌾
- `handleReplaceAll()` - logs replacing all hay with count 🌾
...
**UI Tasks:**
- [x] Rename "Guinea Pigs" FAB to "Interact" with 💛 yellow heart icon
...
- [x] Change inventory sidebar button icon to something more generic
  - Changed from 📦 to ☰ (hamburger menu)

**Interaction Actions (7 total):**
- 🫳 Pet
- 🫴 Hold
- 🥕 Hand Feed (needs food selection dialog)
- 🧼 Gentle Wipe
- 🧸 Show Toy
- 👀 Peek-a-Boo
- 💬 Talk To
...
- [x] Two-step flow: Select Pet → Click guinea pig → Hand animates
- [x] Hand descends from above, pets guinea pig, rises back up
- [x] Animation duration: 3.125 seconds (descend → stroke → rise)
- [x] Hand rotation: palm faces down, fingers perpendicular to floor
- [x] Activity logging with 🫳 emoji
...
- [ ] Hook up need fulfillment for remaining interactions:
  - [ ] Hold → social need (+20-30%)
  - [ ] Hand Feed → hunger need (based on food), social need (+10%)
  - [ ] Gentle Wipe → cleanliness need (+40-50%), social need (+5%)
  - [ ] Show Toy → play need (+10-15%), social need (+5%)
  - [ ] Peek-a-Boo → social need (+10%)
  - [ ] Talk To → social need (+10-15%)
- [ ] Implement hand feed food selection dialog
- [ ] Wire up actual interaction logic for remaining 6 actions:
  - [ ] Hold (🫴)
  - [ ] Hand Feed (🥕)
  - [ ] Gentle Wipe (🧼)
  - [ ] Show Toy (🧸)
...
**Files Modified:**
- `src/components/game/Inventory3DPanel.vue` - Guinea Pigs tab content with User Friendship and GP-to-GP Bonding sections
