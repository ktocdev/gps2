...
**Implementation:**
- Create `use3DThirstBehavior.ts` (similar to `use3DHungerBehavior.ts`)
- Add water level state to `habitatConditions` store
- Update water bottle 3D model to show dynamic water level
- Add bubble particle effect during drinking
- Create `WaterBottleMenu.vue` component for click interaction
- Wire up click detection in `Habitat3DDebug.vue`

**Reference:**
- [use3DHungerBehavior.ts](../../src/composables/3d/use3DHungerBehavior.ts) - Pattern for autonomous behavior
- 2D thirst implementation for threshold values

**Files to Create:**
- `src/composables/3d/use3DThirstBehavior.ts` - Thirst behavior logic
- `src/components/game/WaterBottleMenu.vue` - Water bottle interaction menu

**Files to Modify:**
- [Habitat3DDebug.vue](../../src/components/debug/environment/Habitat3DDebug.vue) - Add thirst behavior, water bottle click
- [habitatConditions.ts](../../src/stores/habitatConditions.ts) - Water level state
- Water bottle 3D model composable - Dynamic water level rendering
...
13. [ ] Implement Drink Action (Thirst Behavior)
