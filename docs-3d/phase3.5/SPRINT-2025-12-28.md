...
- [x] Interaction handlers → `use3DInteractions.ts` composable
- [x] Container menu logic → `use3DContainerMenu.ts` composable
- [x] Placement mode logic → `use3DPlacement.ts` composable
- [x] FAB habitat care handlers → `use3DHabitatCare.ts` composable
- [x] Water bottle menu → `use3DWaterBottle.ts` composable

**Status:** COMPLETE - All extraction candidates have been moved to composables.
...
### True Fullscreen for Mobile/Tablet
**Priority:** MEDIUM (P2)
**Category:** UI/UX

**Goal:** Use browser Fullscreen API for immersive mobile/tablet experience

**Decisions Needed:**
- [ ] Landscape mobile: Use true fullscreen (yes)
- [ ] Landscape tablet: Use true fullscreen (probably yes)
- [ ] Desktop: Keep panel debug page wrapper (no fullscreen API)
- [ ] Portrait mobile: Show "Rotate Screen" message

**Implementation:**
- [ ] Move pause/exit fullscreen buttons into GameView as overlay FABs
- [ ] Detect device type and orientation
- [ ] Use `document.documentElement.requestFullscreen()` on mobile/tablet landscape
- [ ] Handle fullscreen exit events
- [ ] Add "Rotate your device" message for portrait orientation on mobile

**Current Workaround:** `touch-action: none` prevents scroll while in CSS fullscreen mode
