...
- [ ] **Z-Index Issue**: ConfirmDialog appears below panel elements due to lack of proper stacking context
- [ ] **Dialog Refactoring Needed**: Extract base Dialog component using native `<dialog>` element
  - Create `src/components/basic/Dialog.vue` using HTML5 `<dialog>` element
  - Refactor ConfirmDialog to extend base Dialog
  - Ensures proper modal behavior with native backdrop and top-layer rendering
  - Enables future dialog variants (AlertDialog, PromptDialog, CustomDialog)
  - Native `<dialog>` provides automatic focus management, ESC key handling, and accessibility
...
- [x] **Favorite Slot Price Escalation Too High**: ✅ **FIXED** (October 3, 2025)
