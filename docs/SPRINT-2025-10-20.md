...
- ✅ **Behavior Threshold System** - Fixed threshold sliders not affecting AI decisions
  - Created [autonomySettingsStore.ts](../src/stores/autonomySettingsStore.ts) for centralized threshold storage
  - Updated [gameTimingStore.ts:109-120](../src/stores/gameTimingStore.ts#L109-L120) to read and pass custom thresholds to behavior.tick()
  - Connected [AutonomyDebug.vue](../src/components/debug/environment/AutonomyDebug.vue) sliders to store
  - Guinea pigs now respond to custom threshold settings (e.g., 80% hunger threshold works correctly)
- ✅ **Shelter Positioning System** - Fixed guinea pigs not entering shelters/beds
  - Added `"blocksMovement": false` to all 13 hideaway/bed items in [hideaways.json](../src/data/supplies/habitat/hideaways.json)
  - Implemented `preferAnchor` parameter for shelter positioning (true=on top, false=adjacent)
  - Updated [useGuineaPigBehavior.ts:330](../src/composables/game/useGuineaPigBehavior.ts#L330) - seek_shelter uses preferAnchor=true
  - Guinea pigs now properly position on/in shelters and beds
- ✅ **Activity State Centralization** - Fixed tooltip showing wrong activity
  - Created [behaviorStateStore.ts](../src/stores/behaviorStateStore.ts) for centralized behavior state
  - Converted [useGuineaPigBehavior.ts:62-73](../src/composables/game/useGuineaPigBehavior.ts#L62-L73) to use computed property reading from store
  - Updated [GuineaPigSprite.vue](../src/components/game/habitat/GuineaPigSprite.vue) to read directly from behaviorStateStore
  - Tooltips and debug panels now show consistent activity states (sleeping, hiding, eating, etc.)
- ✅ **Interruptible Behaviors** - Fixed behaviors freezing/blocking for long durations
  - Reduced max sleep duration from 200s to 30s ([useGuineaPigBehavior.ts:734](../src/composables/game/useGuineaPigBehavior.ts#L734))
  - Implemented 250ms cancellation checks in sleep, shelter, and hide behaviors
  - Added 10-second timeouts to all `movement.onArrival()` promises
  - Cancel button now responds within 250ms (4x faster than before)
- ✅ **Shelter Threshold Configuration** - Added shelter need threshold controls
  - Changed default shelter threshold from 60% to 50%
  - Added shelter threshold slider to AutonomyDebug panel ([AutonomyDebug.vue:151-173](../src/components/debug/environment/AutonomyDebug.vue#L151-L173))
  - Added "🏠 Seek Shelter" manual trigger button
- ✅ **Shelter Activity Display** - Fixed shelter behavior showing "idle" instead of "hiding"
  - Updated [useGuineaPigBehavior.ts:960](../src/composables/game/useGuineaPigBehavior.ts#L960) to set currentActivity='hiding'
  - Added 'hiding' to BehaviorState.currentActivity union type
  - Shelter-related behaviors now properly display "hiding" in tooltips and debug panels
- ✅ **Habitat Debug Layout** - Added Needs Panel and improved responsive layout
  - Created [NeedsPanel.vue](../src/components/debug/environment/NeedsPanel.vue) with multi-guinea-pig toggle
  - Updated [HabitatDebug.vue](../src/components/debug/environment/HabitatDebug.vue) grid to 3 columns (Autonomy 0.75fr, Needs 1fr, Conditions 1fr)
  - Improved visual hierarchy and spacing for better readability

**Files Created:**
- `src/stores/autonomySettingsStore.ts` - Centralized behavior threshold management per guinea pig
- `src/stores/behaviorStateStore.ts` - Centralized behavior state for consistent activity display
- `src/components/debug/environment/NeedsPanel.vue` - Needs display panel with multi-guinea-pig support

**Files Modified:**
- `src/composables/game/useGuineaPigBehavior.ts` - Interruptible behaviors, state centralization, positioning fixes
- `src/stores/gameTimingStore.ts` - Threshold integration with AI tick system
- `src/components/debug/environment/AutonomyDebug.vue` - Threshold sliders, seek shelter button, store integration
- `src/components/game/habitat/GuineaPigSprite.vue` - Behavior state store integration for tooltips
- `src/data/supplies/habitat/hideaways.json` - Added blocksMovement: false to all items
- `src/components/debug/environment/HabitatDebug.vue` - 3-column layout with NeedsPanel integration
