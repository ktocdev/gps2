...
**Modified Files:**
- `src/composables/game/useGuineaPigBehavior.ts` - Interruptible behaviors, positioning, shorter sleep
- `src/stores/autonomySettingsStore.ts` - Updated default thresholds
- `src/stores/petStoreManager.ts` - Preference generation from catalog, starting needs
- `src/stores/gameTimingStore.ts` - Threshold integration
- `src/components/debug/environment/AutonomyDebug.vue` - Store integration, shelter slider
