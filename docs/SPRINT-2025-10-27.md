...
##### Simultaneous Pooping Bug 🔥 **HIGH PRIORITY**
**Priority:** HIGH
**Status:** 📋 Not Started

**Issue:** Multiple guinea pigs sometimes poop at the exact same time

**Observed Behavior:**
- Two guinea pigs poop simultaneously
- Unrealistic behavior
- May indicate shared timer or race condition

**Expected Behavior:**
- Each guinea pig should have independent poop timing
- 30-second interval should be per guinea pig, not global

**Investigation Required:**
- Verify each guinea pig has independent `lastPoopTime`
- Check if poop interval calculation is using shared state
- Review autonomous pooping logic for race conditions

**Files to Check:**
- `src/composables/game/useGuineaPigBehavior.ts` - `checkAutonomousPooping()` function
- `src/stores/guineaPigStore.ts` - Verify `lastPoopTime` is per guinea pig

**Possible Causes:**
- Shared timer instead of per-guinea-pig timer
- Initialization setting same `lastPoopTime` for all guinea pigs
- Behavior ticks synchronized too closely

---

##### Pause Doesn't Stop Movement 🔥 **HIGH PRIORITY**
**Priority:** CRITICAL
**Status:** 📋 Not Started
...
**Expected Behavior:**
- Pause should immediately freeze all guinea pig movement
- Guinea pigs should stop mid-path
- No behaviors should execute while paused

**Current Behavior:**
- Guinea pigs finish their current path
- Tasks complete even when paused
- Game state continues to update

**Implementation Required:**
- Add pause state check in movement tick
- Add pause state check in behavior execution
- Freeze all timers when paused
- Resume from exact position when unpaused

**Files to Modify:**
- `src/composables/game/useMovement.ts` - Stop movement on pause
- `src/composables/game/useGuineaPigBehavior.ts` - Stop behavior execution on pause
- `src/stores/gameController.ts` - Pause state management
...
##### Activity Messages Timing Lag 📋 **MEDIUM PRIORITY**
**Priority:** MEDIUM
**Status:** 📋 Not Started
...
**Expected Behavior:**
- Message should appear when activity starts or during activity
- User should see real-time updates

**Current Behavior:**
- Message appears after guinea pig has moved on to next task
- Confusing to user about what guinea pig is doing

**Investigation Required:**
- Check when messages are logged (start vs end of behavior)
- Verify timing of message generation in behavior execution
- Consider adding progress messages for long activities

**Files to Check:**
- `src/composables/game/useGuineaPigBehavior.ts` - Message timing in all behavior functions

---

##### Item Tooltips Cut Off 📋 **MEDIUM PRIORITY**
**Priority:** MEDIUM
**Status:** 📋 Not Started
...
##### Tooltip Z-Index Issues 📋 **MEDIUM PRIORITY**
**Priority:** MEDIUM
**Status:** 📋 Not Started
...
##### Duplicate Tooltips on Chew Items 📋 **LOW PRIORITY**
**Priority:** LOW
**Status:** 📋 Not Started
...
##### Shelter Need Not Increasing 📋 **MEDIUM PRIORITY**
**Priority:** MEDIUM
**Status:** 📋 Not Started

**Issue:** When guinea pig goes into a shelter, the shelter need does not increase

**Expected Behavior:**
- Entering shelter should satisfy shelter need
- Shelter need should increase when guinea pig uses hideaway/shelter items
- Should provide security/comfort benefit

**Current Behavior:**
- Guinea pig enters shelter (seeks shelter behavior executes)
- Shelter need remains unchanged or doesn't increase appropriately
- May not be rewarding shelter-seeking behavior

**Investigation Required:**
- Check shelter behavior execution
- Verify need adjustment in shelter behavior
- Check if shelter items are properly detected
- Review shelter need restoration values

**Files to Check:**
- `src/composables/game/useGuineaPigBehavior.ts` - Shelter behavior execution
- Look for shelter need adjustment in `executeShelterBehavior()`
...
#### Contextual Object Messages 📋 **MEDIUM PRIORITY**
**Priority:** MEDIUM
**Status:** 📋 Not Started
...
- 📋 Play - Add toy name
- 📋 Shelter - Add shelter name
- 📋 Chew - Add chew item name
...
#### Water Level Tooltip & Refill 📋 **LOW PRIORITY**
**Priority:** LOW
**Status:** 📋 Not Started
...
#### Activity Log Height Limit 📋 **LOW PRIORITY**
**Priority:** LOW
**Status:** 📋 Not Started
...
- 🔥 **Simultaneous pooping bug** (HIGH - unrealistic timing)
